...
| `/det` | خطة اختبار Duolingo English Test (للكبار) مع جدول متابعة واستراتيجيات الاختبار. مركز المذاكرة في صفحة مستقلة ومرتبة ويحتوي مفردات مترجمة و22 موضوعا وقطعا ونماذج كتابة وتحدث وقواعد وقسم اختبارات تفاعلي. التقدم والدرجات تُحفظ في المتصفح |
...
الموقع يخدم من فرع **`gh-pages`** (يحوي `index.html` و`study.html` و`det-study.js` و`det-study.css` و`.nojekyll`)، ويتولى
[`.github/workflows/pages.yml`](.github/workflows/pages.yml) مزامنته تلقائيًا من
`public/det.html` و`public/study.html` وملفات مركز المذاكرة عند كل دفعة إلى `main` — فالتعديل في مكان واحد ينعكس على
